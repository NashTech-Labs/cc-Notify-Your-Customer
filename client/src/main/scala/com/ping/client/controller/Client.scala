...
import com.ping.models.{ClientRequest, HttpResponse}
...
class Client @Inject()(val clientService: ClientService) extends Controller with JsonHelper {
  ...
  /*get("/:token") { aToken =>
    clientService.getClientByAccessToken(aToken.toString) map {
      ...
      case NonFatal(_) =>
        ...
        response.internalServerError(write(resp)).contentTypeJson()
    }
