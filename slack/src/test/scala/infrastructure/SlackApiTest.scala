...
class SlackApiTest extends WordSpecLike with MockitoSugar {
  ...
  "Slack Api " should {
    ...
    "not send Slack Message when no channel found" in {
      ...
      val res = Await.result(SlackApiTestObj.send(PingSlack("1",Some("general"), "hellooo !!!")), Duration(5, "seconds"))
      assert(res === false)
    }
    "send Slack Message" in {
      ...
      val res = Await.result(SlackApiTestObj.send(PingSlack("1",Some("general"), "sending message on slack !! :)")), Duration(5, "seconds"))
      assert(res === true)
    }
    ...
    "not send Slack Message due to incorrect channel name" in {
      ...
      val res = Await.result(SlackApiTestObj.send(PingSlack("1",Some("channel"), "sending message on slack !! :)")), Duration(20, "seconds"))
      assert(res === false)
    }
  }
}
