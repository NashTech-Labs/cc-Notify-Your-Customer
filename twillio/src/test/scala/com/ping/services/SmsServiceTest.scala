...
import com.ping.models.sms.SmsDetail
...
class SmsServiceTest extends WordSpecLike with MustMatchers with MockitoSugar {
  ...
  "Sms Service " should {
    ...
    "send notification with sms " in {
      ...
      val response = MockSmsServiceTestObject.send(SmsDetail(List("+919671701006"), "Hello Customer"))
    ...
    "not send notification with sms " in {
      ...
      val response = MockSmsServiceTestObject.send(SmsDetail(List("+919910630243"), "Your flight was just support+%s@%s.(1/1)###fight got disrupted####"))
    ...
    "correctly split message in 152 chars" in {
      ...
      val response = MockSmsServiceTestObject.send(SmsDetail(List("+919671701006"), longMessage))
      response mustBe Map("+919671701006" -> true)
    }
