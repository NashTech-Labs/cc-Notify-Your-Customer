...
class SmsServiceTest extends AsyncWordSpecLike with  MustMatchers with MockitoSugar {
  ...
  "Sms Service " should {
    ...
    "send notification with sms " in {
      ...
      val response: Future[Boolean] = MockSmsServiceTestObject.send(TwilioMessage("1", "+919671701006", "Hello Customer"))
    ...
    "not send notification with sms " in {
      ...
      val response = MockSmsServiceTestObject.send(TwilioMessage("2", "+919910630243", "+%s@%s.(1/1)### hey ####"))
      response.map{ res =>
        assert(res ==true )
    }
  }
    ...
    "correctly split message in 152 chars" in {
      ...
      val response = MockSmsServiceTestObject.send(TwilioMessage("3", "+919671701006", longMessage))
      response.map{ res =>
        assert(res ==true )
      }
    }
